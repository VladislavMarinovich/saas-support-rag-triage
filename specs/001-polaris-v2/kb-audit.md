...
| `connectors-reauthorize-expired::how-to-reconnect` | rank 2 · 0.589 · `man-10` | **C + defecto de índice** | **Prioridad**: es el chunk esperado de 4 queries del corpus y las 2 que fallan Recall@5 (`typo-03`, `typo-04`) lo tienen fuera del top-5. *Precisión de la revisión Watson:* el robo del chunk-basura (§3) aplica a `typo-04` (footer rank 5 con 0.632 vs esperado rank 6 con 0.631) y a `es-09`; en `typo-03` NO hay footer en el top-5 — su fallo es puro gap de vocabulario ("oauth") y lo resuelve BM25, no el saneamiento. Se resuelve al sanear el índice; sin vocabulario de jerga ("oauth", "token", "reautorizar") el gap persiste → candidato claro de BM25 (POL-7). |
