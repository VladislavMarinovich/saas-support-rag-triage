...
| **`chunk_dominance_top1_ratio`** — porcentaje de requests donde el chunk más frecuente aparece como top-1 sobre ventana móvil 7 días. Widget: stat con valor absoluto del chunk dominante. | > 10% durante 30 días con muestra > 500 queries. | Chunk imán detectado. Descubrimiento empírico Fase 0.b: `dashboards-not-loading::1.-check-your-internet-connection` apareció como top-1 en 4/30 queries (13%), sesgando respuestas ambiguas hacia "verifica tu internet". Acciones: re-chunkear el candidato para dividir contenido demasiado genérico, agregar contexto que reduzca superposición semántica, o considerar boost negativo controlado en RRF. |
